...
from models.lbs import Struct, to_tensor, to_np, lbs, vertices2landmarks, JointsFromVerticesSelector, find_dynamic_lmk_idx_and_bcoords,  find_dynamic_lmk_idx_and_bcoords_projective
...
class SMPLX(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.smplx_model = Struct(**ss)

        self.dtype = torch.float32
        self.register_buffer('faces_tensor', to_tensor(to_np(self.smplx_model.f, dtype=np.int64), dtype=torch.long))
        # The vertices of the template model
        self.register_buffer('v_template', to_tensor(to_np(self.smplx_model.v_template), dtype=self.dtype))
        # The shape components and expression
        # expression space is the same as FLAME
        shapedirs = to_tensor(to_np(self.smplx_model.shapedirs), dtype=self.dtype)
        from utils.config import cfg
        self.head_ind = np.load(cfg.model.flame_ids_path)
        
        shapedirs = torch.cat([shapedirs[:,:,:config.n_shape], shapedirs[:,:,300:300+config.n_exp]], 2)

        self.register_buffer('shapedirs', shapedirs)
        # The pose components
        num_pose_basis = self.smplx_model.posedirs.shape[-1]
        posedirs = np.reshape(self.smplx_model.posedirs, [-1, num_pose_basis]).T
        self.register_buffer('posedirs', to_tensor(to_np(posedirs), dtype=self.dtype)) 
        self.register_buffer('J_regressor', to_tensor(to_np(self.smplx_model.J_regressor), dtype=self.dtype))
        parents = to_tensor(to_np(self.smplx_model.kintree_table[0])).long(); parents[0] = -1
        self.register_buffer('parents', parents)
        self.register_buffer('lbs_weights', to_tensor(to_np(self.smplx_model.weights), dtype=self.dtype))
        # for face keypoints
        self.register_buffer('lmk_faces_idx', torch.tensor(self.smplx_model.lmk_faces_idx, dtype=torch.long))
        self.register_buffer('lmk_bary_coords', torch.tensor(self.smplx_model.lmk_bary_coords, dtype=self.dtype))
        self.register_buffer('dynamic_lmk_faces_idx', torch.tensor(self.smplx_model.dynamic_lmk_faces_idx, dtype=torch.long))
        self.register_buffer('dynamic_lmk_bary_coords', torch.tensor(self.smplx_model.dynamic_lmk_bary_coords, dtype=self.dtype))
        ...
        from utils.vertex_joint_selector import VertexJointSelector
        from utils.vertex_ids import vertex_ids
        self.vertex_joint_selector = VertexJointSelector(vertex_ids=vertex_ids['smplx'])
        ...
        self.use_joint_regressor = False
    ...
    def cut_flame_head(self, verts):
        ...
        for mesh_face in numpy_faces:
            if (mesh_face[0] in self.head_ind) and (mesh_face[1] in self.head_ind) and (mesh_face[2] in self.head_ind):
                tensor_list.append(mesh_face)
        unsqueezed_tensor = [torch.tensor(i).unsqueeze(0) for i in tensor_list]
        faces_tensor = torch.cat(unsqueezed_tensor)
        mapping = dict(zip(self.head_ind, np.arange(5023).tolist()))
        head_faces = torch.tensor([mapping[i.item()] for i in faces_tensor.reshape(-1)]).reshape(-1, 3)
        return head_verts, head_faces
        

    def forward(self, shape_params=None, expression_params=None,
                    global_pose=None, body_pose=None,
                    jaw_pose=None, eye_pose=None,
                    left_hand_pose=None, right_hand_pose=None, additional_scalp_blendshape=None):
        """
            Args:
...
#             ic(batch_size)
        ...
        vertices, joints = lbs(shape_components, full_pose, template_vertices,
                          ...
                          pose2rot = False, additional_scalp_blendshape =additional_scalp_blendshape)
